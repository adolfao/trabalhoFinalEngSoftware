...
Foram identificadas algumas ambiguidades, principalmente sobre como o sistema deve priorizar horários e lidar com preferências dos professores. Também foram encontrados conflitos como:

professores em dois horários ao mesmo tempo;
disciplinas do mesmo período no mesmo horário;
pouca disponibilidade de horários para algumas matérias.

Além disso, algumas questões ainda ficaram em aberto para as próximas etapas, como a lógica usada para gerar automaticamente a grade e o que fazer quando não existir uma solução sem conflitos.
